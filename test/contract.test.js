...
test('every path the manifest advertises actually exists', () => {
  ...
  for (const entry of pkg.files ?? []) {
    assert.ok(fs.existsSync(path.join(root, entry)), `\`files\` lists "${entry}" but it is missing`)
  }
  for (const [name, target] of Object.entries(pkg.exports ?? {})) {
    ...
    if (target.includes('*')) {
      ...
      assert.ok(fs.existsSync(path.join(root, dir)), `export "${name}" points into missing "${dir}"`)
      continue
    }
